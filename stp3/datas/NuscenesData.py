...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def get_input_data(self, rec):
        ...
        for cam in cameras:
            ...
            img = Image.open(image_filename)      # size: W x H

            #######################################################
            # 修改下面的代码来是你的模型需要的图像预处理方式
            img1 = img   # size (1600, 900)
            # 将 PIL Image 对象转换为 NumPy 数组  
            img_array = np.array(img1)  
            # 转换 NumPy 数组的轴顺序以匹配 PyTorch Tensor 的期望顺序 (C, H, W)  
            img_array_transposed = np.transpose(img_array, (2, 0, 1))  
            # 将 NumPy 数组转换为 PyTorch Tensor  
            img_tensor = torch.from_numpy(img_array_transposed)
            # 定义预处理方式
            tfs_val = tfs.Compose([tfs.Resize([450, 800]), tfs.CenterCrop([384, 704])])
            # 图像预处理
            img_tensor1 = tfs_val(img_tensor)
            images_2.append(img_tensor1.unsqueeze(0).unsqueeze(0))
            #######################################################
        ...
        images, intrinsics, extrinsics, images_2 = (torch.cat(images, dim=1),
                                          torch.cat(intrinsics, dim=1),
                                          torch.cat(extrinsics, dim=1),
                                          torch.cat(images_2, dim=1)
                                          )
        ###############################################################
        if len(depths) > 0:
            depths = torch.cat(depths, dim=1)
        ...
        return images, intrinsics, extrinsics, depths, images_2
        ########################################################
    ...
    def __getitem__(self, index):
        ...
        for i, index_t in enumerate(self.indices[index]):
            ...
            if i < self.receptive_field:
                ...
                images, intrinsics, extrinsics, depths, images_2 = self.get_input_data(rec)
                #############################################################################
        ...
        for key, value in data.items():
            ...
            if key in ['image', 'intrinsics', 'extrinsics', 'depths', 'segmentation', 'instance', 'future_egomotion', 'hdmap', 'pedestrian', 'images_2']:
            ###############################################################################################################################################
